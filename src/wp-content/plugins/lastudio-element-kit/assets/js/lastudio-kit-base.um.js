...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const getHeaderHeight = () => {
        let currentHeight = 0;
        let stickySection = document.querySelector('.elementor-location-header .e-container[data-settings*="sticky_on"]');
        if(stickySection){
...
(function ($) {
    ...
    const fixHeaderTransparency = () => {
        let $stickyContainers = $('.elementor-location-header >.elementor-top-section > .e-con-inner > .elementor-element.e-con[data-settings*="sticky_on"], .elementor-location-header >.elementor-top-section > .elementor-element.e-con[data-settings*="sticky_on"]');
        $stickyContainers.each( ( idx, item ) => {
            $(item).closest('.elementor-top-section').addClass('ignore-docs-style');
        } )
        $stickyContainers.on('sticky:stick', (e) => {
            let $parent = $(e.target).closest('.elementor-top-section');
    ...
    const LaStudioKits = {
        ...
        initCarousel: function ($scope) {
            ...
            function findAsForObj(value, key) {
                let _found = [];
                for (let i = 0; i < LaStudioKits.carouselAsFor.length; i++) {
            ...
            function makeLoadedAsFor(value, key) {
                let obj = findAsForObj(value, key);
                if (obj.length) {
            ...
            if (typeof elementSettings.asFor !== "undefined" && elementSettings.asFor !== '' && elementSettings.asFor !== '#' && $('#' + elementSettings.asFor).length > 0) {
            ...
            new Swiper($swiperContainer, swiperOptions).then(function (SwiperInstance) {
                ...
                const ob1 = makeLoadedAsFor(carousel_id, 'thumb');
                const ob2 = makeLoadedAsFor(carousel_id, 'main');

                if (ob1.length && ob1[0].main_init && ob1[0].thumb_init) {
                    let _main_swiper_instance = $('#' + ob1[0].main).data('swiper');
                    let _thumb_swiper_instance = $('#' + ob1[0].thumb).data('swiper');
                    _main_swiper_instance.controller.control = _thumb_swiper_instance;
                    _thumb_swiper_instance.controller.control = _main_swiper_instance;
                }
                if (ob2.length && ob2[0].main_init && ob2[0].thumb_init) {
                    let _main_swiper_instance = $('#' + ob2[0].main).data('swiper');
                    let _thumb_swiper_instance = $('#' + ob2[0].thumb).data('swiper');
                    _main_swiper_instance.controller.control = _thumb_swiper_instance;
                    _thumb_swiper_instance.controller.control = _main_swiper_instance;
                }
