...
//        \wc_set_loop_prop('is_filtered', \is_filtered());
            ...
            switch ($setting['item_type']){
                ...
                case 'product_countdown':
                    $item_html = $this->v2_loop_countdown($el_class, [
                        'days' => !empty($setting['countdown_label_day']) ? $setting['countdown_label_day'] : '',
                        'hours' => !empty($setting['countdown_label_hour']) ? $setting['countdown_label_hour'] : '',
                        'minutes' => !empty($setting['countdown_label_minute']) ? $setting['countdown_label_minute'] : '',
                        'seconds' => !empty($setting['countdown_label_second']) ? $setting['countdown_label_second'] : '',
                    ]);
                    $html .= apply_filters('lastudio-kit/products/zone_item_html', $item_html, $el_class, $setting);
                    break;
    ...
    private function v2_loop_stock( $elClass = '', $icon = '', $type = 'label', $label = ''){
        ...
        if($type == 'bar'){
            if(empty($label)){
                $label = __('%1$s sold/ %2$s total', 'lastudio-kit');
            }
            ...
            if($stock_available > 0){
                $bar_label = sprintf($label, $stock_sold, $stock_available);
                $stock_bar = sprintf(
    ...
    private function v2_loop_countdown( $elClass = '', $labels = [] ){
        ...
        if( $product->is_on_sale() ){
            ...
            if(!empty($sale_price_dates_to)){
                ...
                $html = sprintf(
                    '<div class="%1$s lakitp-zone-item product_item--countdown"><div class="lakit-countdown-timer" data-due-date="%2$s">%3$s</div></div>',
                    $elClass,
                    $sale_price_dates_to,
                    ($day . $hour . $min . $sec)
                );
